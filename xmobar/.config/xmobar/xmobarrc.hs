...
import Xmobar
    ( defaultConfig,
      xmobar,
      Border(NoBorder),
      Config(template, overrideRedirect, font, additionalFonts, bgColor,
             fgColor, position, border, lowerOnStart, commands, sepChar,
             alignSep),
      XPosition(Top),
      Command(Com),
      Date(Date),
      Locks(Locks),
      Monitors(Uptime, DiskU, MultiCpu, CoreTemp, Memory, Network),
      XMonadLog(XMonadLog),
      Runnable(Run) )
...
config = defaultConfig {
        ...
        , commands = [  Run Locks
         ...
         , sepChar = "%"
         , alignSep = "}{"
         , template = "%XMonadLog% }{ %locks% %disku% HOLA %multicpu% %coretemp% %memory% %eno1% %gpu% %upgradeable% %speakers% %headphones% %uptime% %date% <fn=1>🖖</fn>"
         }
